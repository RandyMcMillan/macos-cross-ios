...
BOOL AppDel_didFinishLaunching(struct AppDel *self, SEL _cmd, void *application, void *options)
{
    ...
    self->window = cmacs_simple_msgSend(objc_getClass("UIWindow"), sel_getUid("alloc"));
    self->window = cmacs_rect_msgSendCGRect(self->window, sel_getUid("initWithFrame:"),
                                            (struct CGRect) { 0, 0, 320, 480 });
    ...
    //NOTE: ...alloc] init]; normally nested
    id viewController = (cmacs_simple_msgSend((id)objc_getClass("UIViewController"), sel_getUid("alloc")));
       viewController = (cmacs_simple_msgSend(viewController, sel_getUid("init")));
    ...
    id view = cmacs_simple_msgSend((id)objc_getClass("View"), sel_getUid("alloc"));
       view = cmacs_rect_msgSendCGRect(view, sel_getUid("initWithFrame:"),
                                            (struct CGRect) { 0, 0, 320, 480 });
    
    // here we simply add the view to the view controller, and add the viewController to the window.
    cmacs_void_msgSendVoid(cmacs_simple_msgSend(viewController, sel_getUid("view")), sel_getUid("addSubview:"), view);

    cmacs_void_msgSendVoid(self->window, sel_getUid("setRootViewController:"), viewController);
    
    // finally, we display the window on-screen.
    cmacs_simple_msgSend(self->window, sel_getUid("makeKeyAndVisible"));
